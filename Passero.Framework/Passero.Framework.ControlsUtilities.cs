...
namespace Passero.Framework
{
...

    {
        ...
        /// <summary>
        /// Imposta ReadOnly=true sui controlli bindati via BindingSource alle proprietà
        /// che costituiscono la chiave primaria del modello <typeparamref name="T"/>.
        /// Funziona scorrendo i <see cref="Binding"/> del <see cref="BindingSource.CurrencyManager"/>
        /// e confrontando <see cref="Binding.BindingMemberInfo.BindingField"/> con i nomi
        /// delle proprietà marcate [Key] o [ExplicitKey] nel modello.
        /// </summary>
        /// <typeparam name="T">Tipo del modello associato al BindingSource.</typeparam>
        /// <param name="bindingSource">BindingSource collegato ai controlli.</param>
        /// <param name="readOnly">
        /// <c>true</c> per rendere i controlli PK in sola lettura (default),
        /// <c>false</c> per ripristinarli editabili.
        /// </param>
        /// <returns>Elenco dei controlli modificati.</returns>
        public static IReadOnlyList<Control> SetPrimaryKeyControlsReadOnly<T>(
        ...
        /// <summary>
        /// Versione non-generica di <see cref="SetPrimaryKeyControlsReadOnly{T}"/>.
        /// Usa il tipo del modello ricavato a runtime da <paramref name="modelType"/>
        /// oppure direttamente dalla lista del <paramref name="bindingSource"/>.
        /// </summary>
        /// <param name="bindingSource">BindingSource collegato ai controlli.</param>
        /// <param name="modelType">
        /// Tipo del modello (es. <c>typeof(Models.Publisher)</c>).
        /// Se <c>null</c> viene inferito dalla lista del BindingSource.
        /// </param>
        /// <param name="readOnly">
        /// <c>true</c> per rendere i controlli PK in sola lettura,
        /// <c>false</c> per ripristinarli editabili.
        /// </param>
        /// <returns>Elenco dei controlli modificati.</returns>
        public static IReadOnlyList<Control> SetPrimaryKeyControlsReadOnly(
            BindingSource bindingSource,
            Type modelType = null,
            bool readOnly = true)
        {
            var affected = new List<Control>();

            if (bindingSource == null)
                return affected;

            // Inferisce il tipo del modello dalla lista del BindingSource se non passato
            modelType ??= ReflectionHelper.GetListType(bindingSource.List);
            if (modelType == null)
                return affected;
            ...
            foreach (Binding binding in currencyManager.Bindings)
            {
                // BindingMemberInfo.BindingField è il nome della proprietà del modello
                var fieldName = binding.BindingMemberInfo.BindingField;
                if (!pkNames.Contains(fieldName))
                    continue;
                ...
                // Imposta ReadOnly tramite reflection per supportare qualsiasi
                // tipo di controllo (TextBox, DateTimePicker, ComboBox, ecc.)
                var readOnlyProp = control.GetType()
                    .GetProperty("ReadOnly", BindingFlags.Public | BindingFlags.Instance);
                //var backColorProp = control.GetType()
                //   .GetProperty("BackColor", BindingFlags.Public | BindingFlags.Instance);
                //var foreColorProp = control.GetType()
                //   .GetProperty("ForeColor", BindingFlags.Public | BindingFlags.Instance);

                if (readOnlyProp != null && readOnlyProp.CanWrite)
                {
                    readOnlyProp.SetValue(control, readOnly);
                    //if (backColorProp != null && backColorProp.CanWrite)
                    //{
                    //    backColorProp.SetValue(control, BackColor);
                    //}
                    //if (foreColorProp != null && foreColorProp.CanWrite)
                    //{
                    //    foreColorProp.SetValue(control, ForeColor);
                    //}
