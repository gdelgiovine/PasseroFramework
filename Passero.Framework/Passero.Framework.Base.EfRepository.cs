...
namespace Passero.Framework
{
        ...
        where ModelClass : class
    {
        ...
        public EfRepository(IPasseroDbContext dbContext)
        {
            ...
            DefaultSQLQuery = $"SELECT * FROM {GetTableName()}";
        }
            ...
            int? commandTimeout = null)
        {
            ...
            try
            {
                SQLQuery = query;
                var sqlParams = BuildSqlParameters(parameters);
                ApplyResults(_dbContext.SqlQuery<ModelClass>(query, sqlParams).ToList());
                er.Value = ModelItems;
                er.DebugInfo = query;
            }
            catch (Exception ex)
            {
                HandleError(er.ToExecutionResult(), ex, query);
            }
            LastExecutionResult = er.ToExecutionResult();
            return er;
        }
            ...
            int? commandTimeout = null)
        {
            ...
            try
            {
                ...
                var result = await Task.Run(() =>
                    _dbContext.SqlQuery<ModelClass>(query, sqlParams).ToList());
                ApplyResults(result);
                er.Value = ModelItems;
                er.DebugInfo = query;
            }
            ...
            int? commandTimeout = null)
        {
            ...
            try
            {
                SQLQuery = query;
                var sqlParams = BuildSqlParameters(parameters);
                var item = _dbContext.SqlQuery<ModelClass>(query, sqlParams).SingleOrDefault();
            ...
            int? commandTimeout = null)
        {
            ...
            try
            {
                ...
                var item = await Task.Run(() =>
                    _dbContext.SqlQuery<ModelClass>(query, sqlParams).SingleOrDefault());
                ModelItem = item ?? GetEmptyModelItem();
                SetModelItemShadow();
                er.Value = ModelItem;
                er.DebugInfo = query;
