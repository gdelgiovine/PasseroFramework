    ...
    public class PasseroEf6DbContext : Ef6.DbContext, IPasseroDbContext
    {
        ...
        protected override void OnModelCreating(Ef6.DbModelBuilder modelBuilder)
        {
            ...
            foreach (var entityType in allTypes)
            {
                ...
                // ── Chiave primaria via [ExplicitKey] di Dapper ──────────────
                // Per chiave singola senza [Key]: HasKey(e => e.Prop) via Fluent API.
                // Per chiave composta: i modelli devono avere [Key, Column(Order=N)]
                // sulle proprietà chiave — EF6 le riconosce nativamente senza HasKey.
                ...
                {
                    // Chiave singola: HasKey(e => e.Prop)
                    var param = Expression.Parameter(entityType, "e");
                    var propExpr = Expression.Property(param, explicitKeyProps[0]);
                    var keyLambda = Expression.Lambda(propExpr, param);
                    ...
                    {
                        var hasKeyMethodClosed = hasKeyMethodOpen.MakeGenericMethod(keyLambda.ReturnType);
                        hasKeyMethodClosed.Invoke(entityConfig, new object[] { keyLambda });
                    }
                }
                // Chiave composta (Length > 1): EF6 deduce la PK da [Key]+[Column(Order=N)]
                // definiti sui modelli. Non serve HasKey via Fluent API.
                ...
                // ── Mapping colonna: [Column("nome")] → HasColumnName ─────────
                // Necessario affinché EF6 mappi correttamente il data reader
                // anche nelle raw query (Database.SqlQuery<T>).
                ApplyColumnMappings(entityType, entityConfig);
            }
        ...
        private static void ApplyColumnMappings(Type entityType, object entityConfig)
        {
            foreach (var prop in entityType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                bool isComputed = prop.GetCustomAttribute<Dapper.Contrib.Extensions.ComputedAttribute>() != null;
                ...
                if (string.Equals(columnAttr.Name, prop.Name, StringComparison.Ordinal))
                    continue;
                ...
                try
                {
                    ...
                    object primitiveConfig = null;
                    var propType = prop.PropertyType;

                    // string → Property(Expression<Func<TEntity, string>>) — non generico
                    if (propType == typeof(string))
                    {
                        ...
                        var lambda = System.Linq.Expressions.Expression.Lambda(funcType, propExpr, param);

                        var stringMethod = entityConfig.GetType()
                            .GetMethods()
                        ...
                        primitiveConfig = stringMethod?.Invoke(entityConfig, new object[] { lambda });
                    }
                    // nullable value type → Property<T>(Expression<Func<TEntity, T?>>) con T : struct
                    else if (propType.IsGenericType && propType.GetGenericTypeDefinition() == typeof(Nullable<>))
                    {
                        ...
                        var nullableMethod = entityConfig.GetType()
                            .GetMethods()
                            .FirstOrDefault(m => m.Name == "Property"
                                                 && m.IsGenericMethodDefinition
                                                 && m.GetParameters().Length == 1);
                        ...
                        {
                            var closed = nullableMethod.MakeGenericMethod(innerType);
                            primitiveConfig = closed.Invoke(entityConfig, new object[] { lambda });
                        }
                    }
                    // value type non nullable → Property<T>(Expression<Func<TEntity, T>>) con T : struct
                    else if (propType.IsValueType)
                    {
                        var funcType = typeof(Func<,>).MakeGenericType(entityType, propType);
                        var lambda = System.Linq.Expressions.Expression.Lambda(funcType, propExpr, param);

                        var valueMethod = entityConfig.GetType()
                            .GetMethods()
                            .FirstOrDefault(m => m.Name == "Property"
                                                 && m.IsGenericMethodDefinition
                                                 && m.GetParameters().Length == 1);
                        ...
                        if (valueMethod != null)
                        {
                            var closed = valueMethod.MakeGenericMethod(propType);
                            primitiveConfig = closed.Invoke(entityConfig, new object[] { lambda });
                        }
                    }
                    ...
                    var hasColumnNameMethod = primitiveConfig.GetType()
                        .GetMethod("HasColumnName", new[] { typeof(string) });
                    hasColumnNameMethod?.Invoke(primitiveConfig, new object[] { columnAttr.Name });
                }
                catch (Exception ex)
                {
                    // Proprietà non scalare (navigazione, collezione): ignorata silenziosamente.
                    throw;
                }
            }
        }
