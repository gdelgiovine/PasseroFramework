...
namespace Passero.Framework.DapperHelper
{
    ...
    public class Utilities
    {
        ...
        /// <summary>
        /// Returns the mapped column name for a property, respecting <see cref="Dapper.ColumnMapper.ColumnMappingAttribute"/> when present.
        /// Falls back to the property name if no mapping attribute is found.
        /// </summary>
        ...
        /// <summary>
        /// Gets the update SQL command, respecting column name mappings via <see cref="Dapper.ColumnMapper.ColumnMappingAttribute"/>.
        /// </summary>
        /// <param name="ModelClass">The model class.</param>
        /// <returns></returns>
        public static string GetUpdateSqlCommand(Type ModelClass)
        {
            ...
            string wherelist;
            string sql;
            properties = Utilities.GetPropertiesInfo(ModelClass, true);
            ...
            sql = $"UPDATE {Utilities.GetTableName(ModelClass)} SET {setlist} WHERE ({wherelist})";
            return sql;
        }
        ...
        /// <summary>
        /// Gets the INSERT SQL command for the given model type,
        /// respecting column name mappings via <see cref="Dapper.ColumnMapper.ColumnMappingAttribute"/>.
        /// Identity keys ([Key]) are excluded from the column list; explicit keys ([ExplicitKey]) are included.
        /// Computed and non-writable properties are excluded.
        /// Appends SELECT CAST(SCOPE_IDENTITY() AS BIGINT) to allow retrieving the generated identity value.
        /// </summary>
        /// <param name="ModelClass">The model class type.</param>
        /// <returns>A parameterized INSERT SQL string.</returns>
        public static string GetInsertSqlCommand(Type ModelClass)
        {
            var properties = Utilities.GetPropertiesInfo(ModelClass, true)
            ...
            return $"INSERT INTO {DapperHelper.Utilities.GetTableName(ModelClass)} ({columns}) VALUES ({parameters}); SELECT CAST(SCOPE_IDENTITY() AS BIGINT)";
        }
        ...
        /// </summary>
        /// <param name="ModelClass">The model class type.</param>
        /// <returns>A parameterized DELETE SQL string.</returns>
        public static string GetDeleteSqlCommand(Type ModelClass)
        {
            var keys = DapperHelper.Utilities.GetPrimaryKeysPropertiesInfo(ModelClass);
            var where = string.Join(" AND ", keys.Select(pi => $"{GetMappedColumnName(pi)}=@{pi.Name}"));
            return $"DELETE FROM {DapperHelper.Utilities.GetTableName(ModelClass)} WHERE ({where})";
        }
